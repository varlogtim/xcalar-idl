window.DagPanel = (function($, DagPanel) {
    ...
    function setupDagTableDropdown() {
        ...
        $dagPanel.on('click', '.dagTable', function(event) {
            ...
            TableComponent.getMenu().showDagAndTableOptions($menu, null, $dagTable);
    ...
    function setupDagOpDropdown() {
        ...
        $dagPanel.on('click', '.operationType', function(event) {
                            ...
                            for (var i = 0; i < node.value.tags.length; i++) {
                                if (node.value.tags[i].indexOf(SQLOps.Union) === 0 ||
                                    ...
                                    if (allDagInfo.tagGroups[tagId] &&
                                        ...
                                        hideEdit = true;
                                        if (node.value.tags[i]
                                            .indexOf(SQLOps.Union) === 0) {
                                            hideOp = SQLOps.Union;
            ...
            if (DagEdit.isEditMode()) {
                ...
                if ($opWrap.hasClass("union") || $opWrap.hasClass("join")) {
            ...
            if (!Object.keys(edits.structs).length &&
                ...
                Alert.show({
                    ...
                    "onConfirm": function() {
                        ...
                        DagFunction.runProcedureWithParams(tableName,
                            edits.structs, edits.newNodes, edits.insertedNodes);
                    }
                });
            }
        });
