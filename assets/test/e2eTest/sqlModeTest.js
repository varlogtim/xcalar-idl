...
module.exports = {
    ...
    after: function(browser) {
        console.log("=====",browser.globals.gTestWorkbookName);
        browser.deleteWorkbook(browser.globals.gTestWorkbookName, testConfig.user);
    },
        ...
        testConfig.queries.forEach((query) => {
            browser
                ...
                .execute(function(query) {
                    ...
                    SQLEditorSpace.Instance.newSQL(query.sql);
                }, [query])
                .click("#sqlEditorSpace .execute")
                .waitForElementVisible("#sqlTableArea", 600000)
                .execute(function(query) {
                    const tableArea = document.getElementById("#sqlTableArea");
                    for (let row in query.result) {
                        let answers = query.result[row];
                        if (row === "numOfRows") {
                            const numOfRows = tableArea.getElementsByClassName(
                                                    ".totalRows")[0].innerText;
                            expect(numOfRows).to.be.equal(answers);
                        } else {
                            for (let i = 0; i < answers.length; i++) {
                                const col = "col" + (i + 1);
                                const res = tableArea.querySelectorAll("." + row
                                  + " ." + col + " .originalData")[0].innerText;
                                if (typeof answers[i] === "number") {
                                    browser.expect(Math.abs(answers[i].toFixed(2)
                                                    - parseFloat(res).toFixed(2))
                                                    .toFixed(2)).to.be.most(0.01); 
                                } else if (answers[i] === null) {
                                    browser.expect(res).to.be.equal("FNF");
                                } else {
                                    browser.expect(answers[i]).to.be.equal(res);
                                }
                            }
                        }
                    }
