describe('DSExport', function() {
	...
	describe('XcalarListExportTargets', function() {
		it('should return expected struct', function(done) {
			...
			.then(function(targs) {
				...
				return XcalarRemoveExportTarget(testTargetName, ExTargetTypeT.ExTargetSFType);
			})
			.then(function() {
			...
			.then(function(targs) {
				...
				return XcalarRemoveExportTarget(testTargetName, ExTargetTypeT.ExTargetUDFType);
			})
			.then(function() {
				done();
			})
	...
	describe('reset', function() {
		it('reset should work', function() {
			...
			expect($form.find('.udfFuncListWrap').css('pointer-events')).to.equal("none");
	...
	describe('form submit with invalid inputs', function() {
		...
		describe('blank target type', function() {
			it ('blank target type should trigger error', function(done) {
				...
				expect($("#statusBox").text().indexOf(ErrTStr.NoEmptyList)).to.be.gt(-1);
				StatusBox.forceHide();
				DSExport.__testOnly__.resetForm();
			});
		});
		...
		describe('blank path', function() {
			it('blank path should produce error', function(done) {
				...
				expect($("#statusBox:visible").length).to.equal(1);
				expect($("#statusBox").text().indexOf(ErrTStr.NoEmpty)).to.be.gt(-1);
				StatusBox.forceHide();
				DSExport.__testOnly__.resetForm();
			});
		});
	...
	describe('successful form submit', function() {
		...
		it('localFileSystem should work', function(done) {
			var numGrids = $("#gridTarget-LocalFileSystem").find('.grid-unit').length;
			expect($('.grid-unit[data-name="' + testTargetName + '"]').length).to.equal(0);

			submitForm("LocalFilesystem", testTargetName, "url", {})
			.then(function() {
				var newNumGrids = $("#gridTarget-LocalFileSystem").find('.grid-unit').length;
	...
	describe('grid panel', function() {
		...
		it('grid icon should show details', function(done) {
			...
			.then(function() {
				...
				expect($("#exportTargetEditCard .udfModuleName").val()).to.equal('a');
				expect($("#exportTargetEditCard .udfFuncName").val()).to.equal('b');
		...
		it('grid icon menu should work', function(done) {
			...
			expect($menu.find('[data-action="delete"]:visible').length).to.equal(1);
