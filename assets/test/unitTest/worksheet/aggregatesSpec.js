describe('Aggregates', function() {
	...
	describe('general functionality', function() {
		...
		it('adding and getting aggs should work', function() {
			...
			var isTemp = false;
			Aggregates.addAgg(testId, testCol, aggOp, aggRes, isTemp);

			var agg = Aggregates.getAgg(testId, testCol, aggOp)
			expect(agg).to.be.an('object');
			expect(agg).to.have.all.keys('key', 'value', 'dagName', 'aggName', 'tableId');
			expect(agg.key).to.equal(testId + "#" + testCol + "#" + aggOp);
	...
	describe('testing with backend', function() {
		...
		it('aggregate should be added when xcFunction.aggregate is called', function(done) {
			Aggregates.clear();
			expect(Aggregates.getAggs()).to.be.empty;
			xcFunction.aggregate(null, tableId, 'count', prefix + "::yelping_since", "^namedAgg")
			.then(function() {
				var aggs = Aggregates.getAggs();
				expect(aggs).to.not.be.empty;
				...
				expect(aggs.namedAgg.backColName).to.equal(prefix + "::yelping_since");
		...
		it('aggregate should show on agg list in menu panel', function(done) {
			setTimeout(function() {
				Alert.forceClose();
				var listItem = $("#constantList").find('li.tableInfo[data-id="namedAgg"]');
		...
		it('aggregates deleteAggs should work', function(done) {
			...
			.then(function() {
				...
				.then(function() {
					listItem = $("#constantList").find('li.tableInfo[data-id="namedAgg"]');
					expect(listItem.length).to.equal(0);
				})
				.fail(function() {
					expect('tablelist refresh failed').to.equal('tableList refresh succeeded');
				})
				.always(function() {
					done();
				});
