...
describe("PTblManager Test", function() {
    ...
    describe("createTableFromDataset Test", function() {
        let dsName;
        let tableName;
        let oldRefresh;
        let oldCreateTable;
        ...
        it("should create a table from dataset", function(done) {
            PTblManager.Instance.createTableFromDataset(dsName, tableName, [], [])
            .then(function(res) {
        ...
        it("should reject error case", function(done) {
            PTblManager.Instance._createTable = () => PromiseHelper.reject("test");
            PTblManager.Instance.createTableFromDataset(dsName, tableName, [], [])
            .then(function() {
                done("fail");
            })
            .fail(function(error) {
                expect(error).to.equal("test");
                expect(PTblManager.Instance._datasetTables[tableName]).to.equal(undefined);
                done();
            });
        });

        it("should reject error case 2", function(done) {
            let tableInfo = PTblManager.Instance.createTableInfo(tableName);
            PTblManager.Instance._datasetTables[tableName] = tableInfo;
            PTblManager.Instance._createTable = () => PromiseHelper.reject("test");
            PTblManager.Instance.createTableFromDataset(dsName, tableName, [], [])
            ...
            .fail(function(error) {
                expect(error).to.equal("test");
                expect(PTblManager.Instance._datasetTables[tableName]).to.equal(tableInfo);
                delete PTblManager.Instance._datasetTables[tableName];
                done();
            });
        });
