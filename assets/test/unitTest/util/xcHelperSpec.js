describe("xcHelper Test", function() {
    ...
    it('xcHelper.parseEvalString(str) should work', function() {
        var fn = xcHelper.parseEvalString;
        var func;
        var str;
        var desiredStr;
        ...
        str = "add(1,2)";
        func = fn(str);
        desiredStr = '{"fnName":"add","args":["1","2"]}';
        expect(JSON.stringify(func)).to.equal(desiredStr);

        str = "add  (1,3)";
        func = fn(str);
        desiredStr = '{"fnName":"add","args":["1","3"]}';
        expect(JSON.stringify(func)).to.equal(desiredStr);

        str = "add(1  ,4)";
        func = fn(str);
        desiredStr = '{"fnName":"add","args":["1","4"]}';
        expect(JSON.stringify(func)).to.equal(desiredStr);

        str = "add ( 1  , 5  )";
        func = fn(str);
        desiredStr = '{"fnName":"add","args":["1","5"]}';
        expect(JSON.stringify(func)).to.equal(desiredStr);

        str = 'concat ("wo rd",5)';
        func = fn(str);
        desiredStr = '{"fnName":"concat","args":["\\\"wo rd\\\"","5"]}';
        expect(JSON.stringify(func)).to.equal(desiredStr);

        str = 'concat (\'wo"r"a"d\',5)';
        func = fn(str);
        desiredStr = '{"fnName":"concat","args":["\'wo\\\"r\\\"a\\\"d\'","5"]}';
        expect(JSON.stringify(func)).to.equal(desiredStr);

        str = 'con\\"c\\,at (\'w\\,o"r\\\'d\',5)';
        func = fn(str);
        desiredStr = '{"fnName":"con\\\\\\"c\\\\,at","args":["\'w\\\\,o\\"r\\\\\'d\'","5"]}';
        expect(JSON.stringify(func)).to.equal(desiredStr);

        str = 'concat ("wo\\"rd",6)';
        func = fn(str);
        desiredStr = '{"fnName":"concat","args":["\\"wo\\\\\\"rd\\"","6"]}';
        expect(JSON.stringify(func)).to.equal(desiredStr);

        str = 'concat ("w\'o\\"rd",7)';
        func = fn(str);
        desiredStr = '{"fnName":"concat","args":["\\"w\'o\\\\\\"rd\\"","7"]}';
        expect(JSON.stringify(func)).to.equal(desiredStr);

        str = 'add(1e2,7)';
        func = fn(str);
        desiredStr = '{"fnName":"add","args":["1e2","7"]}';
        expect(JSON.stringify(func)).to.equal(desiredStr);

        str = 'add(0xFF,8)';
        func = fn(str);
        desiredStr = '{"fnName":"add","args":["0xFF","8"]}';
        expect(JSON.stringify(func)).to.equal(desiredStr);

        str = 'add(null,9)';
        func = fn(str);
        desiredStr = '{"fnName":"add","args":["null","9"]}';
        expect(JSON.stringify(func)).to.equal(desiredStr);

        str = "map(add())";
        func = fn(str);
        desiredStr = '{"fnName":"map","args":[{"fnName":"add","args":[]}]}';
        expect(JSON.stringify(func)).to.equal(desiredStr);
        ...
        str = "map(add(1,))";
        func = fn(str);
        desiredStr = '{"fnName":"map","args":[{"fnName":"add","args":["1",""]}]}';
        expect(JSON.stringify(func)).to.equal(desiredStr);

        str = "map(add(1,,2))";
        func = fn(str);
        desiredStr = '{"fnName":"map","args":[{"fnName":"add","args":["1","","2"]}]}';
        expect(JSON.stringify(func)).to.equal(desiredStr);

        str = "map(add((1),,2))";
        func = fn(str);
        desiredStr = '{"fnName":"map","args":[{"fnName":"add","args":[{"fnName":"","args":["1"]},"","2"]}]}';
        expect(JSON.stringify(func)).to.equal(desiredStr);

        str = "map(add((1)))";
        func = fn(str);
        desiredStr = '{"fnName":"map","args":[{"fnName":"add","args":[{"fnName":"","args":["1"]}]}]}';
        expect(JSON.stringify(func)).to.equal(desiredStr);

        str = 'map( add(1,con cat ("ab", "cd" )) )';
        func = fn(str);
        desiredStr = '{"fnName":"map","args":' +
                        '[{"fnName":"add","args":' +
                            '["1",{"fnName":"con cat","args":["\\"ab\\"","\\"cd\\""]}]' +
                        '}]' +
                    '}';
        expect(JSON.stringify(func)).to.equal(desiredStr);
