...
(function () {
    ...
    Topic.ScrollToBookmark = function (id) {
        ...
        if ($target.length > 0) {
            ...
            var $menus = $("ul.menu.mc-component");
            if ($menus.length > 0 && !($menus[0].innerHTML.replace(/\s/g, '').length)) {
                $menus.on("loaded", function () {
    ...
    function ScrollToOffset(targetOffset) {
        if ($(".title-bar.sticky").length > 0) {
            var titleBarOffset = $(".title-bar.sticky").innerHeight();
            $("html, body").scrollTop(targetOffset - titleBarOffset);
        } else {
