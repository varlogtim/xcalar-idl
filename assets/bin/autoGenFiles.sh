...
echo "var gBuildNumber = '"${BUILD_NUMBER}"';" >> assets/js/setup/constructorVersion.js
echo -n "var gConstructorVersion = '" >> assets/js/setup/constructorVersion.js

OS=`uname`
if [ "${OS}" = "Darwin" ]; then
    echo -n `md5 prod/assets/js/setup/constructor.js | cut -d' ' -f4` >> assets/js/setup/constructorVersion.js
else
    echo -n `md5sum prod/assets/js/setup/constructor.js | cut -d' ' -f4` >> assets/js/setup/constructorVersion.js
fi
