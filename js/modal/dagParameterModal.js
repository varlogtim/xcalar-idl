window.DagParamModal = (function($, DagParamModal){
    ...
    DagParamModal.show = function($currentIcon) {
        ...
        var $dummyInputs = $dagParamModal.find('.dummy');
    ...
    DagParamModal.paramDragStart = function(event) {
        ...
        // $dagParamModal.find('.dummy').show();
        $dagParamModal.find('input.editableParamDiv').each(function() {
            var width = $(this).width();
            $(this).siblings('.dummyWrap').show().width(width);
            $(this).hide();
        });
        // $dagParamModal.find('input.editableParamDiv').hide();
        var val;
        var valLen;
        var html = "";
        $dagParamModal.find('input.editableParamDiv').each(function() {
            ...
            for (var i = 0; i < valLen;i++){
              html += ('<span class="line" ondragover="DagParamModal.allowParamDrop(event)" ' +
                'ondrop="DagParamModal.paramDropLine(event)">' + val[i] + '</span>');
            }
            html += ('<span class="space hover" ondragover="DagParamModal.allowParamDrop(event)" ' +
                'ondrop="DagParamModal.paramDropSpace(event)"></span>');
    ...
    DagParamModal.paramDragEnd = function (event) {
        ...
        $dagParamModal.find('.dummyWrap').hide();
        $dagParamModal.find('input.editableParamDiv').show();
    };
    ...
    DagParamModal.paramDropLine = function(event) {
        event.stopPropagation();
        var $dropTarget = $(event.target);
        ...
        // $dropTargParent.text($dropTargParent.text() + $draggableParam.text());
    ...
    DagParamModal.paramDropSpace = function(event) {
        event.stopPropagation();
        var $dropTarget = $(event.target);
        var $dropTargParent = $dropTarget.parent();
        var paramId = event.dataTransfer.getData("text");
